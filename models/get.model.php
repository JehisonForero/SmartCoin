...
class GetModel{
	...
	static public function getRelDataRange($rel,$type,$select,$linkTo,$between1,$between2,$orderBy,$orderMode,$startAt,$endAt, $filterTo, $inTo){
		...
		/*=============================================
			permite agrupar informacion prueba cesar 
		=============================================*/
        ...
        // Aplica los límites de inicio y fin si son proporcionados
        if ($startAt >= 0 && $endAt > $startAt) {
            $sql .= " LIMIT $startAt, " . ($endAt - $startAt);
        }

        // Ejecuta la consulta SQL utilizando PDO (debes establecer tu conexión a la base de datos previamente)
        $stmt = Connection::connect()->prepare($sql);
        $stmt->execute();
    ...
    /* prueba seleccion datos relacionados agrupados */
	  static public function getGrupData($rel, $type, $select, $orderBy,$orderMode,$startAt,$endAt){
		...
		if(count($relArray)>1){
			...
			foreach ($relArray as $key => $value) {
				...
				if($key > 0){
					...
					if ($key == (count($typeArray) - 1)) {
					    $innerJoinText .= "GROUP BY ".$relArray[0].".id_".$typeArray[$key]."_".$typeArray[0];
					}
